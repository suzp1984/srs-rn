...
function render() {
  ...
  const route = {key: 'WhepUrl-test', name: 'WhepUrl' as const} as never;
  let tree!: ReactTestRenderer.ReactTestRenderer;
  ReactTestRenderer.act(() => {
    tree = ReactTestRenderer.create(<WhepUrlScreen navigation={navigation} route={route} />);
  });
  return {tree, navigate};
}

describe('WhepUrlScreen', () => {
  it('prefills the input with DEFAULT_WHEP_URL on mount', () => {
    const {tree} = render();
    expect(findByTestID(tree.root, 'whep-url-input').props.value).toBe(DEFAULT_WHEP_URL);
  });
  it('navigates to Player with the default URL when Connect is tapped', () => {
    ...
    expect(navigate).toHaveBeenCalledWith('Player', {whepUrl: DEFAULT_WHEP_URL});
  });
  ...
  it('shows an error for non-http(s) URLs and does not navigate', () => {
    const {tree, navigate} = render();
    ReactTestRenderer.act(() => {
      findByTestID(tree.root, 'whep-url-input').props.onChangeText('ftp://host/x');
    });
    ReactTestRenderer.act(() => {
      findByTestID(tree.root, 'whep-url-connect').props.onPress();
    });
  ...
  it('keeps the URL editable', () => {
    ...
    expect(input.props.value).toBe('https://example.com/whep');
  });
});
