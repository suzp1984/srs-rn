...
import type {FlvErrorEvent, FlvSessionStatus, UseFlvSessionResult} from './types';
...
const noop = () => {};
const noopError = (_event: FlvErrorEvent) => {};

// Mirrors useRtmpSession/useDashSession (explicit Start, attemptRef + isCurrent
// guard, idempotent cleanupAttempt via a closed flag, unmount teardown) on the
// same libVLC media stack. libVLC's FLV demuxer and HTTP access module are core
// modules (unlike SRT's external libsrt), so the HTTP-FLV stream is pulled
...
export function useFlvSession(): UseFlvSessionResult {
  const [status, setStatus] = React.useState<FlvSessionStatus>('idle');
